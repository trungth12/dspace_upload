...
<!--
    dongpn-mobile.xsl
    Mobile theme for DSpace 1.6, 1.7, 4.0
    Last edit by Phan Ngoc Dong
    Last update: 12, 2013
...
<xsl:stylesheet 
    ...
    <xsl:template match="dri:document">
        <html>
            ...
            <body>
                <div data-role="page" data-fetch="always"> <!-- id="ds-main" -->
                    <!--
 ...
 <!-- The HTML head element contains references to CSS as well as embedded JavaScript code. Most of this
    ...
    <xsl:template name="buildHead">
	    ...
	    <META NAME = "author" CONTENT= "Phan Ng&#7885;c &#272;&#244;ng" />
	    ...
	    <!-- custom CSS -->
			...
			<xsl:value-of select="/dri:document/dri:meta/dri:pageMeta/dri:metadata[@element='contextPath'][not(@qualifier)]"/>
                    <xsl:text>/themes/</xsl:text>
		     <xsl:value-of select="/dri:document/dri:meta/dri:pageMeta/dri:metadata[@element='theme'][@qualifier='path']"/>
                    <xsl:text>/lib/sc-mobile.min.css</xsl:text>
		    ...
		    <xsl:value-of select="/dri:document/dri:meta/dri:pageMeta/dri:metadata[@element='contextPath'][not(@qualifier)]"/>
                    <xsl:text>/themes/</xsl:text>
		     <xsl:value-of select="/dri:document/dri:meta/dri:pageMeta/dri:metadata[@element='theme'][@qualifier='path']"/>
                    <xsl:text>/lib/jquery.mobile.css</xsl:text>
		...
		<!-- CSS tweaks -->
		 ...
		 <xsl:attribute name="href">
			<xsl:value-of select="/dri:document/dri:meta/dri:pageMeta/dri:metadata[@element='contextPath'][not(@qualifier)]"/>
                    <xsl:text>/themes/</xsl:text>
		     <xsl:value-of select="/dri:document/dri:meta/dri:pageMeta/dri:metadata[@element='theme'][@qualifier='path']"/>
                        <xsl:text>/lib/m-tweaks.css</xsl:text>
                    </xsl:attribute>
                </link>
		 ...
		 <xsl:attribute name="href">
			<xsl:value-of select="/dri:document/dri:meta/dri:pageMeta/dri:metadata[@element='contextPath'][not(@qualifier)]"/>
                    <xsl:text>/themes/</xsl:text>
		     <xsl:value-of select="/dri:document/dri:meta/dri:pageMeta/dri:metadata[@element='theme'][@qualifier='path']"/>
                        <xsl:text>/lib/custom.css</xsl:text>
                    </xsl:attribute>
                </link>
	...
	<script>
	 ...
	 <xsl:value-of select="/dri:document/dri:meta/dri:pageMeta/dri:metadata[@element='contextPath'][not(@qualifier)]"/>
        ...
        <xsl:text>/lib/custom.js</xsl:text>
	...
	<script>
	 ...
	 <xsl:value-of select="/dri:document/dri:meta/dri:pageMeta/dri:metadata[@element='contextPath'][not(@qualifier)]"/>
        ...
        <xsl:text>/lib/jquery.latest.js</xsl:text>
        </xsl:attribute>&#160;</script>
	...
	<script>
	 ...
	 <xsl:value-of select="/dri:document/dri:meta/dri:pageMeta/dri:metadata[@element='contextPath'][not(@qualifier)]"/>
        ...
        <xsl:text>/lib/jquery.mobile.js</xsl:text>
        </xsl:attribute>&#160;</script>
	 ...
	 <xsl:value-of select="/dri:document/dri:meta/dri:pageMeta/dri:metadata[@element='contextPath'][not(@qualifier)]"/>
        ...
        <xsl:text>/lib/cookies.js</xsl:text>
        </xsl:attribute>&#160;</script>
	...
	<div data-role="header"  data-theme="b"> 
		  <i18n:text>xmlui.dri2xhtml.structural.header-promotional</i18n:text>
			<a href="/mobile" data-icon="home" data-iconpos="notext" data-ajax="false">Trang ch&#7911;</a>
			<a href="javascript:history.go(-1)" data-icon="back" data-iconpos="notext" data-direction="reverse" class="ui-btn-right" data-ajax="false">Quay l&#7841;i</a>
			<div data-role="navbar" data-iconpos="top">
			<ul>
				<li>
					<a href="/mobile" data-theme="" data-icon="grid" >
						Trang ch&#7911;
					</a>
				</li>
				
				<li>
					<a href="/mobile/password-login" data-theme="" data-icon="star">
						&#272;&#259;ng nh&#7853;p
					</a>
				</li>
				<li>
					<a href="/mobile/discover" data-theme="" data-icon="search">
						T&#236;m ki&#7871;m
					</a>
				</li>
			</ul>
...
<!--
        ...
        <div data-role="header">
            ...
            <h2> 
                <xsl:text>TH&#431; VI&#7878;N \u0110I\u1EC6N T\u1EEC TR&#431;&#7900;NG &#272;&#7840;I H&#7884;C &#272;&#192; L&#7840;T</xsl:text>
   	        </h2>
            <a>
                <xsl:attribute name="href">
...
<xsl:apply-templates />
	<xsl:choose>
    	<xsl:when test="/dri:document/dri:meta/dri:pageMeta/dri:metadata[@element='title']='xmlui.general.dspace_home'">
           ...
           <form id="search" class="ds-interactive-div primary" action="/mobile/discover" method="get">
               <fieldset>
               <input id="search-basic" name="query" type="search" value="" />
               <input id="search" name="submit" type="submit" value="T&#236;m" data-icon="check" data-iconpos="right"/>
               </fieldset>
           </form>
<!--
		...
		<a>
                <xsl:attribute name="href">
                <xsl:text>/mobile/advanced-search</xsl:text>
                </xsl:attribute>
                <xsl:attribute name="data-role">
                <xsl:text>button</xsl:text>
                </xsl:attribute>
                
                <xsl:text>T&#236;m ki&#7871;m n&#226;ng cao</xsl:text>
            </a>
	  ...
	  -->
           ...
           <div id="browse-front-page" data-role="controlgroup">
            ...
            <a>
                <xsl:attribute name="href">
                <xsl:text>/mobile/browse?type=title</xsl:text>
                </xsl:attribute>
                <xsl:attribute name="data-role">
                <xsl:text>button</xsl:text>
                </xsl:attribute>
                ...
                <xsl:text>Nhan &#273;&#7873;</xsl:text>
            </a>

            <a>
                <xsl:attribute name="href">
                <xsl:text>/mobile/browse?type=author</xsl:text>
                </xsl:attribute>
                <xsl:attribute name="data-role">
                <xsl:text>button</xsl:text>
                </xsl:attribute>
                ...
                <xsl:text>T&#225;c gi&#7843;</xsl:text>
            </a>

            <a>
                <xsl:attribute name="href">
                <xsl:text>/mobile/browse?type=dateissued</xsl:text>
                </xsl:attribute>
                <xsl:attribute name="data-role">
                <xsl:text>button</xsl:text>
                </xsl:attribute>
                ...
                <xsl:text>N&#259;m xu&#7845;t b&#7843;n</xsl:text>
            </a>

            <a>
                <xsl:attribute name="href">
                <xsl:text>/mobile/browse?type=subject</xsl:text>
                </xsl:attribute>
                <xsl:attribute name="data-role">
                <xsl:text>button</xsl:text>
                </xsl:attribute>
...
<!-- link to full website -->
        <a href="#" data-role="button" data-icon="forward" data-iconpos="left">
                ...
                <xsl:text>Giao di&#7879;n chu&#7849;n</xsl:text>
            </a>
...
<!-- END front page customization  -->
    ...
    <xsl:template name="buildFooter">
        <div data-role="footer" data-theme="b" >  <!-- id="ds-footer" --> 
...
<div id="addthis">
                <a>
                    <xsl:attribute name="href">
                        <xsl:text>http://www.addthis.com/bookmark.php?v=300&amp;username=ra-4e200027299b4fed</xsl:text>
                    </xsl:attribute>
                    <xsl:attribute name="class">
			...
			<xsl:text>/mobile/themes/dongpn-mobile/lib/images/lg-share-en.gif</xsl:text>
                        </xsl:attribute>
                ...
                <script type="text/javascript">
                    <xsl:attribute name="src">
                        <xsl:text>http://s7.addthis.com/js/300/addthis_widget.js#username=ra-4e200027299b4fed</xsl:text>
                    </xsl:attribute>
...
<div class="metadata-view">
                        <span class="bold">T&#224;i li&#7879;u li&#234;n quan: </span>
                                <xsl:for-each select="dim:field[@element='title'][not(@qualifier)]">
                                ...
                                <a>
                                    ...
                                    <xsl:text>Tr&#234;n Google Scholar</xsl:text>
                                </a>
                            </xsl:for-each>
</div>
            </xsl:if>
...
<!--
<xsl:variable name="url-1" select="substring-after($url-1,'/mobile')"/>
-->
...
<xsl:text>_blank</xsl:text>
                            ...
                            <img alt="Thumbnail">
                                <xsl:attribute name="src">
                                <xsl:text>/mobile/themes/dongpn-mobile/lib/images/sc-default-thumbnail.png</xsl:text>
                                </xsl:attribute>
                            </img>
                        </a>
                    </xsl:otherwise>
...
<!-- BEGIN remove search box front page -->
<xsl:template match="dri:div[@n='front-page-search']">&#160;
</xsl:template>
<!-- END remove search box front page -->

<!-- BEGIN remove community recent submission -->
<xsl:template match="dri:div[@n='community-recent-submission']">&#160;
</xsl:template>
<!-- END remove community recent submission -->


<!-- BEGIN remove ds-options -->
<xsl:template match="dri:options">&#160;
</xsl:template>
<!-- END remove ds-options -->
